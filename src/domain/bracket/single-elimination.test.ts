...
describe("single elimination", () => {
  it.each([
    ...
    [12, 16],
    [32, 32],
  ])("finds bracket size for %i", (count, size) =>
    expect(nextPowerOfTwo(count)).toBe(size),
  );
  ...
  it.each([2, 3, 4, 5, 6, 7, 8, 9, 12, 16, 32])(
    "creates connected matches for %i entrants",
    (count) => {
      ...
      expect(
        bracket.matches
          ...
          .flatMap((match) => [
            match.participantOne.entrantId,
            match.participantTwo.entrantId,
          ])
          .filter(Boolean),
      ).toHaveLength(count);
