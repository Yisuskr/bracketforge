    ...
    matches: [
      {
        ...
        status: "completed",
        players: [
          { seed: 1, name: "Lina Vega", score: 2, winner: true },
          { seed: 8, name: "Álex Mora", score: 0 },
        ],
      },
      {
        id: "M02",
        status: "completed",
        players: [
          { seed: 4, name: "Noah Silva", score: 2, winner: true },
          { seed: 5, name: "Iris León", score: 1 },
        ],
      },
      {
        id: "M03",
        status: "completed",
        players: [
          { seed: 2, name: "Leo Cruz", score: 2, winner: true },
          { seed: 7, name: "Dani Ríos", score: 0 },
        ],
      },
      {
        id: "M04",
        status: "completed",
        players: [
          { seed: 3, name: "Mara Sol", score: 1 },
          { seed: 6, name: "Kai Vidal", score: 2, winner: true },
        ],
      },
    ],
  },
    ...
    matches: [
      {
        ...
        status: "live",
        players: [
          { seed: 1, name: "Lina Vega", score: 1 },
          { seed: 4, name: "Noah Silva", score: 1 },
        ],
      },
      {
        id: "M06",
        status: "ready",
        players: [
          { seed: 2, name: "Leo Cruz" },
          { seed: 6, name: "Kai Vidal" },
        ],
      },
    ],
  },
...
function TrophyIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M8 4h8v5a4 4 0 0 1-8 0V4Zm0 2H5v2a3 3 0 0 0 3 3m8-5h3v2a3 3 0 0 1-3 3M12 13v4m-3 3h6" />
    </svg>
  );
}
...
export function BracketPreview({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className={classNames("bracket-module", compact && "is-compact")}>
      {!compact && (
        <div className="bracket-toolbar" aria-label="Controles del bracket">
          <div
      ...
      <div className="bracket-viewport" ref={viewportRef}>
        <div className="bracket-grid" style={{ transform: `scale(${zoom})` }}>
            ...
            >
              ...
              <div className="round-matches">
                {round.matches.map((match) => (
                  <article
                    className={classNames("match-card", `is-${match.status}`)}
                    key={match.id}
                    tabIndex={0}
                  >
                    <header>
                    ...
                    <div className="players">
                      {match.players.map((player, playerIndex) => (
                        ...
                        >
                          ...
                          {player.winner && (
                            <span className="winner-mark" title="Ganador">
                              ✓
                            </span>
                          )}
                          <b>{player.score ?? "—"}</b>
          ...
          <aside className="champion-card" aria-label="Campeón pendiente">
            ...
            <small>La final está esperando</small>
          </aside>
        </div>
      </div>
    </div>
